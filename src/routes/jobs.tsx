  ...
  return (
    <AppShell>
            ...
            <div key={j.id} className="p-4 flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-1">
                  <img src={poster.avatar} className="h-4 w-4 rounded-full" alt="" />
                  Posted by {poster.name.split(" ")[0]}
                </div>
                <Button size="sm" onClick={() => setTarget(poster)} className="mt-2 rounded-full h-8 bg-accent hover:bg-accent/90 text-accent-foreground text-xs">
                  Ask for Referral
                </Button>
              </div>
      ...
      <section className="bg-card mt-3 p-4 flex items-center gap-3">
        <img src={currentUser.avatar} className="h-12 w-12 rounded-full" alt="" />
        <div className="text-sm font-semibold flex-1">Apply smarter with jobs personalized for you</div>
      </section>
      <IntentSheet open={!!target} onOpenChange={(o) => !o && setTarget(null)} target={target} />
    </AppShell>
  );
}